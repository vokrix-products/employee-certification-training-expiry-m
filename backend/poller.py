...
def main():
    print(f"Poller started for product {PRODUCT_ID}")
    while True:
        try:
            ...
            for job in jobs:
                ...
                try:
                    result_path, message = process(job)
                    update_job(job["id"], status="completed", result_path=result_path, message=message)
                except Exception as e:
                    update_job(job["id"], status="failed", message=str(e))
